...
from PyQt5.QtWidgets import (
    ...
    QPushButton, QLabel, QSlider, QFileDialog, QAction, QMessageBox, QProgressBar
)
...
class CellTrackingGUI(QMainWindow):
    ...
    def update_frame(self, frame_index):
        ...
        img_view.invertY(True)  # Invert Y-axis to match image coordinates
    ...
    def draw_masks_and_trajectories(self, frame_index, img_view):
        ...
        for label in labels_in_frame:
            ...
            y_max, x_max = positions.max(axis=0) + 1
            ...
            if positions is not None:
                ...
                curve = pg.PlotCurveItem(
                    positions[:, 1], positions[:, 0],  # Note: x and y are swapped
    ...
    def get_cell_trajectory(self, label, frame_index):
        ...
        for t in range(cell_info['t_start'], frame_index + 1):
            ...
            if np.any(cell_mask):
                ...
                positions.append((y_mean, x_mean))
        if positions:
            return positions
        else:
            return None
