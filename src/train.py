...
# from src.Notebooks.XBNet import XBNet
...
data_dir = '/home/komal.kumar/Documents/Cell/datasets/pre/GT+ST/'
...
cases = ['BF-C2DL-HSC'] #,'Fluo-N3DH-CHO'] # change the names for the cases used for training
...
if if_resume == 1:
    ...
    with open(os.path.join(result_dir, 'loss_history.pkl'), 'rb') as f:
        loaded_data = pickle.load(f)

    # Unpack the loaded data
    loss_history_CE = loaded_data['loss_history_CE']
    loss_history_JR = loaded_data['loss_history_JR']
    LR_history = loaded_data['LR_history']
    evl_epochs = loaded_data['evl_epochs']
    seg_history_validation = loaded_data['seg_history_validation']
    seg_score_validation_max = loaded_data['seg_score_validation_max']
    seg_score_max_epoch = loaded_data['seg_score_max_epoch']
    time_lapsed = loaded_data['time_lapsed']
    ...
    # Initialize for new training session
...
data_to_save = {
    'loss_history_CE': loss_history_CE,
    'loss_history_JR': loss_history_JR,
    'LR_history': LR_history,
    'evl_epochs': evl_epochs,
    'seg_history_validation': seg_history_validation,
    'seg_score_validation_max': seg_score_validation_max,
    'seg_score_max_epoch': seg_score_max_epoch,
    'time_lapsed': time_lapsed,
}

# Save the data using pickle
with open(os.path.join(result_dir, 'loss_history.pkl'), 'wb') as f:
